...
- `Date IO` / `Moment` - for date management
- `Material UI (MUI)` as UI components library
- `Notistack` - as a library which displays snackbar and toast messages. Migrated from MUI.
- `Cytoscape` - as a graph library
- `axios` and `React Query` as data-access layer tools
- `Zod` as validation library. YUP should be removed in the future in favor of ZOD
