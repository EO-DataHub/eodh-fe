  ...
  if (isPending) {
    return (
      <>
       ...
       <Button 
         size='medium' 
         text={<>{t(`${translationPath}.DELETING`)}<LoadingSpinner size='xs' classNameSpinner='!border-t-bright ml-0.5'/></>} 
         className='!bg-error' 
...
export const DeleteConfirmation = ({onNoClick, deleteHistoryItem, isError, isPending, isSuccess}: IDeleteConfirmationProps) => {
  const { t } = useTranslation();
