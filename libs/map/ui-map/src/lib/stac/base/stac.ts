...
import SourceType from './type.js';
import {
  defaultBoundsStyle,
...
class STACLayer extends LayerGroup {
  ...
  constructor(options) {
    const superOptions = {};
    ['opacity', 'visible', 'zIndex', 'minResolution', 'maxResolution', 'minZoom', 'maxZoom', 'properties'].forEach(
      (key) => (superOptions[key] = options[key])
    );
   ...
   * @private
   */
  async addChildren_(collection: STAC[], options = {}) {
  ...
  async addLayerForLink(link) {
    ...
    switch (link.rel) {
      ...
      case 'wmts':
        ...
        const layers = Array.isArray(link['wmts:layer']) ? link['wmts:layer'] : [link['wmts:layer']];
  ...
  async addLabelExtension_() {
    ...
    if (!labelAsset) {
      assets = data.getAssets().filter((asset) => asset.type === geojsonMediaType && !asset.roles);
      if (assets.length === 1) {
        labelAsset = assets[0];
      }
    }
  ...
  async updateLayers_() {
    ...
    if (this.hasOnlyBounds()) {
      ...
      if (data instanceof APICollection) {
        ...
        if (data.isItem() && data.supportsExtension(LABEL_EXTENSION) && data.getMetadata('label:type') === 'vector') {
